...
abstract contract HandlersApproveAction is BaseHandlers {
  function handler_executeTransaction_ApproveAction(uint256 _seed) public {
  ...
  function handler_queueApproveAction(uint256 _approvalDuration, uint256 _amount) public {
    _approvalDuration = bound(_approvalDuration, 1, 1000);
    _amount = bound(_amount, 1, 1_000_000);

    address actionsBuilder = approveActionFactory.createApproveAction(
      address(actionTarget), // token
      address(signers[0]), // spender
      _amount // amount
    );
