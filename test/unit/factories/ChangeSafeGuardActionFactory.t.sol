...
contract UnitChangeSafeGuardActionFactorycreateChangeSafeGuardAction is Test {
  ...
  function test_WhenCalled(address _safe, address _actionsBuilder, address _safeGuard) external {
    vm.assume(_safe != address(0));
    vm.assume(_actionsBuilder != address(0));
    vm.assume(_safeGuard != address(0));

    address _changeSafeGuardActionContract =
      changeSafeGuardActionFactory.createChangeSafeGuardAction(_safe, _actionsBuilder, _safeGuard);

    auxChangeSafeGuardAction =
      IChangeSafeGuardAction(deployCode('ChangeSafeGuardAction', abi.encode(_safe, _actionsBuilder, _safeGuard)));
    ...
    assertEq(_changeSafeGuardAction.ACTIONS_BUILDER(), _actionsBuilder);
